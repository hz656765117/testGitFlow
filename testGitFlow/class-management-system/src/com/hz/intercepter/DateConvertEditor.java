...
public class DateConvertEditor implements Converter<String, Date> {
	...
	private SimpleDateFormat minuteFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm");
	...
	public Date convert(String source) {
		...
		if (StringUtils.hasText(source)) {
			try {
				...
				}else if(source.indexOf(":") != -1 && source.length() == 16){
					return (this.minuteFormat.parse(source));
