...
<script type="text/javascript" src="${webPath}jsp/basicDataMng/classMng/js/class.js?v=2017030902"></script>
	  ...
	  <div class="landtitle">
	    ...
	    <div class="mb10 mt10 searchVo">
				<form class=" form-inline form-inline-sm">
					<!-- 点击新建的时候是展示另外一个界面 -->
					<a role="button" class="btn btn-primary mr8" href="${webPath}basicDataMng/classMng/toCreateClass.do"  >新建</a>
					...
					<div class="form-group form-group-sm">
						<input class="form-control input-large" id="searchTxt" param="search.content" placeholder="按班级、专业或者学院名称查询">
					</div>
			    ...
			    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="mytable caozuo  mb20 table">
					<tr>
							...
							<th class="text-left" width="10%">班级代码</th>
							<th class="text-left" width="22%">班级名称</th>
							<th class="text-left" width="20%">所属专业</th>
							<th class="text-left" width="20%">所属学院</th>
				   	...
				   	<tr>
 						...
 						{{classKey}}
					      ...
					      {{majorPO.majorName}}
					       ...
					       <td class="text-left"><span class="hui" >
					      {{majorPO.academyPO.academyName}}
					      </span></td>
						  <td class="text-center"><span class="hui">{{createDateStr}}</span></td>
					       <td><p class="clearfix text-right  pr8">
					   		       <a class="ico-del icon-caozuo f0 fr mr10" href="javascript:void(0);" onclick="delConfirm('single','{{classKey}}');" title="删除" >删除</a>
					   		       <a class="ico-edit icon-caozuo f0 fr mr10" href="javascript:void(0);"onclick="getById('{{classKey}}');" title="编辑" >编辑</a>
				   		    </p>
				   		   </td>
					  </tr>
				   {{/obj}}
...
<!-- 选择判断是否的选择面板 -->
 ...
 <script type="text/javascript">
		...
		function delete_all(type,id){
			...
			//执行删除方法
			var url="${webPath}basicDataMng/classMng/deleteClass.do?keyArr="+ids;
		...
		//获取修改面板		
		function getById (id){
			var url="${webPath}basicDataMng/classMng/toUpdateClass.do?classKey="+id;
			location = url;
		}
 </script>