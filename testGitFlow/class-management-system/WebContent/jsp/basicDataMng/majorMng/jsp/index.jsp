...
<script type="text/javascript" src="${webPath}jsp/basicDataMng/majorMng/js/major.js?v=2017030902"></script>
...
<%-- 				<a class="blue f12" style="margin-left:18px;" href="JavaScript:window.open('${webPath}/helps/helps_list_80.html')"><img src="${webPath}images/admin/grayquestion.png" class="ver-3 mr5">使用帮助</a> --%>
	    ...
	    <div class="mb10 mt10 searchVo">
				<form class=" form-inline form-inline-sm">
					<!-- 点击新建的时候是展示另外一个界面 -->
					<a role="button" class="btn btn-primary mr8" href="${webPath}basicDataMng/majorMng/toCreateMajor.do"  >新建</a>
<!-- 					<a href="javascript:void(0);" onclick="delConfirm('all')" class="btn btn-default mr18 hui"><i class="ico-del-link icon-bg">批量删除</i></a> -->
					<div class="form-group form-group-sm">
						<input class="form-control input-large" id="searchTxt" param="search.content" placeholder="按专业或者学院名称查询">
					</div>
			    ...
			    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="mytable caozuo  mb20 table">
					<tr>
							...
							<th class="text-left" width="10%">专业代码</th>
							<th class="text-left" width="25%">专业名称</th>
							<th class="text-left" width="25%">所属学院</th>
				   ...
				   {{#obj}}
				   	<tr>
 						...
 						{{majorKey}}
					      ...
					      <td class="text-left"><span class="hui" >
					      {{academyPO.academyName}}
					      </span></td>
						  <td class="text-center"><span class="hui">{{createDateStr}}</span></td>
					       <td><p class="clearfix text-right  pr8">
					   		       <a class="ico-del icon-caozuo f0 fr mr10" href="javascript:void(0);" onclick="delConfirm('single','{{majorKey}}');" title="删除" >删除</a>
					   		       <a class="ico-edit icon-caozuo f0 fr mr10" href="javascript:void(0);"onclick="getById('{{majorKey}}');" title="编辑" >编辑</a>
				   		    </p>
				   		   </td>
					  </tr>
				   {{/obj}}
...
<!-- 选择判断是否的选择面板 -->
 ...
 <script type="text/javascript">
		...
		function delete_all(type,id){
			...
			//执行删除方法
			var url="${webPath}basicDataMng/majorMng/deleteMajor.do?keyArr="+ids;
		...
		//获取修改面板		
		function getById (id){
			var url="${webPath}basicDataMng/majorMng/toUpdateMajor.do?majorKey="+id;
			location = url;
		}
 </script>