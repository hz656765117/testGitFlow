...
function showpageFun2(obj, page, total, funcName, args) {
	...
	if (page != 1) {
		html += '<a href="javascript:void(0)" class="js-link-page" data-page="' + (page - 1) + '" > < 上一页 </a>';
	} else {
		html += '<span class="disabled" >< 上一页 </span>';
	}
	if (page < 4 || total < 6) {
		...
		for (var i = 1; i <= xh; i++) {
			if (i == page) {
				...
				html += '<a href="javascript:void(0)" class="js-link-page" data-page="' + i + '" class="ft-arial">' + i + '</a>';
			}
		}
		if (total > 5) {
			html += '<a href="javascript:void(0)" class="ft-arial">...</a>';
		...
		html += '<a  href="javascript:void(0)" class="js-link-page" data-page="1" class="ft-arial">' + 1 + '</a>';
		html += '<a href="javascript:void(0)" class="ft-arial">...</a>';
		var isShow = true;
		for (var i = -1; i < 3; i++) {
			var c = parseInt(page) + parseInt(i);
			if (c <= total) {
				if (c == page) {
					...
					html += '<a href="javascript:void(0)" class="js-link-page" data-page="' + c + '" class="ft-arial"> ' + c + ' </a>';
				}
			} else {
				isShow = false;
			}
	...
	// 下一页
	if (page < total) {
		html += '<a  href="javascript:void(0)" class="js-link-page" data-page="' + (page + 1) + '" >下一页></a>';
	} else {
		html += '<span class="disabled" >下一页 ></span>';
	}
	html += "</div>";
	...
	// 绑定事件
	$('.pagination').off().on('click', '.js-link-page', function () {
