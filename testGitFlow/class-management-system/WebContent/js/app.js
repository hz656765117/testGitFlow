...
function renderPagination(data) {
	...
	if ($("#MyTable")) {
		$(".tab-body").empty();
	 	$(".tab-body").html('<table class="table table-bordered table-text-center" id = "MyTable"><tbody id="container"></tbody></table>');
	}
	var output = Mustache.render(Boxjs.template, $.extend(data, formatTem));
	$('#container').html(output);
	var container_height=$('.tab-body').height();
	var container_width=$('.tab-body').width();
	if ($("#MyTable")){
		FixTable("MyTable", 2, container_width, container_height);
	}
}
/**
