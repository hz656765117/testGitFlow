...
function initTree_dept(webPath){
	var setting_visiable = {
		...
		async : {
			enable : true,
			url : webPath+"contacts/getAllNodeDept?type=-999",
			autoParam : [ "id" ],
...
function filter_visiable(treeId, parentNode, childNodes) {
	...
	for (var i = 0, l = childNodes.obj.length; i < l; i++) {
		if(childNodes.obj[i].childCount>0){
...
function sycPerson(id){
	$.ajax({
		type : 'POST',
		url : webPath + 'contacts/getAllNodePerson',
		data : {id: id},
		success : function(msg) {
			 personInfo = eval(msg).obj;
			 var html ="" ;
	      if(personInfo!=null){
...
function setHtml_visiable(webPath,imgPath){
	...
	jQuery(document.body).append(
			...
			+'</ul></div>'
			+'<div class="hide" style="padding: 0"><div class="box box2 demo3 clearfix"><ul id="departmentTree_x210x_person" class=" ztree  tab_menu2" style="height:260px;">'
			+'</ul>'
			+'<div id="personbox" class="tab_box tab_box2" style="overflow:hidden;height:260px;float:none;width:auto" >'
			+'</div></div></div>'
			+'</div></div></div>'
	...
	*/
	...
	function onload(){
	    ...
	    $('.defaultinput').focus(function(){
	    	...
	    	$.ajax({
	    		type : 'POST',
	    		url : '/contacts/getsearchAll',
	    		data : {searchtxt: value.trim(),
	    			    personids:personids,
	    			    departids:departids
	    			    },
	    		success : function(msg) {
	    			perso = eval(msg);
	    			 var html="";
	    			 for(var i=0;i<perso.obj.length;i++){
	    				 var  personInfo=perso.obj[i];
	    				 if(personInfo!=null){
	    					 if(personInfo.id>0){
		    	    	         html+='<li  onclick="clickLiTwo(this,'+personInfo.id+',1)"><span class="matchname" >'+personInfo.name+'</span><span class="info cel">'+personInfo.mobileFir+'</span><span class="info depart">'+personInfo.departmentName+'</span></li>';
	    					}else{
	   	    	    	         html+='<li  onclick="clickLiTwo(this,'+personInfo.departmentId+',0)"><span class="matchname" >'+personInfo.departmentName+'</span></li>';
	    					}
	    				 }
	    			 }
	    		 if(html!=''){
...
var isOK=false;
function saveDept_visible() {
	isOK=true;
	setCheckNodeValue_visible();
	closeDiv("_x210x");
	visible();
}
